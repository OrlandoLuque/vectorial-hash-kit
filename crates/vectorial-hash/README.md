...
| `kdtree3` | `KdTree3<T>` | 3D **median-split** k-d tree (tight per-node AABBs). Unlike the midpoint-split `Tree3`/`Octree3`, it balances by point COUNT, so depth stays ~log₂(n/leaf) on CLUSTERED data where the midpoint trees halve the empty middle and bloat. `from_items`/`cull`/`knn`. Static; measured cull ~2× / k-NN ~1.6× faster than `Tree3` on clustered data (`examples/kdtree3_bench`), ~1.2× on uniform. |
