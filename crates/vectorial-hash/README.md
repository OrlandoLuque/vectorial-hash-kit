...
- **Stable `ItemRef`** (implemented on `Tree`, `Tree3`, `Octree3`): `insert_ref` returns an `ItemRef` handle that stays valid as the item moves between leaves (a parallel per-leaf handle vector + a handle→location slot-map, maintained through splits/merges); `update_ref`/`remove_ref` then reach the item in **O(1)** — no locate walk, no predicate scan. The 3D decision-map sweep showed the predicate `update`'s O(item_limit) leaf scan is what made the trees lose the per-frame relocate race to a flat grid rebuild; the handle path removes it (~5–11× faster maintain, 10× at item_limit 64) and flips the winner back to the trees (`docs/THREE_D.md` § "The fix: Stable ItemRef"). Default `update`/`remove` keep the predicate API for callers without a handle (the Lca path preserves the handle; the Legacy strategy reassigns it). Churn-tested per structure. *Not yet on `QuadTree` / `IntegerTree`.*
