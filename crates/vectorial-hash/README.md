...
**New here?** Ten structures (2D + 3D) answer the same queries — see
...
Mature. **Ten structures** share one arena layout and query surface: `Tree`
...
| `kdtree3` | `KdTree3<T>` | 3D **median-split** k-d tree (tight per-node AABBs). Unlike the midpoint-split `Tree3`/`Octree3`, it balances by point COUNT, so depth stays ~log₂(n/leaf) on CLUSTERED data where the midpoint trees halve the empty middle and bloat. `from_items`/`cull`/`knn`/`raycast`/`serialize`. Static; measured cull ~2.0-2.3x / k-NN 1.67x faster than `Tree3` on clustered data (`examples/kdtree3_bench`, median of 3 passes), ~1.24x on uniform. |
| `kdtree2` | `KdTree2<T>` | The 2D twin: **median-split** k-d tree with tight per-node rects. Completes the family — every other 2D index splits at the midpoint (`Tree`, `QuadTree`, `LinearQuadTree`) or not at all (`MortonGrid`). `from_items`/`from_items_par`/`cull`/`knn`/`raycast`. Static. Measured on 200k clustered points (`examples/linear_quadtree_bench`): **fastest build AND fastest cull** of the 2D set (7.96 ms / 7.42 ms vs QuadTree 33.50 / 11.45, median of 3; cull ratio ~1.62x measured paired), k-NN a hair behind the pointer quadtree; parallel build 2.79x on 16 threads. |
| `linear_octree3` | `LinearOctree3<T>` | 3D **adaptive** linear octree: sparse hash of leaf buckets keyed by a self-describing Morton location code (path + level in one `u64`), pointer-free; a leaf splits into 8 only where points cluster. `from_items`/`insert`/`cull`/`knn`/`serialize`/`raycast` + parallel batch `cull_many_par`/`knn_many_par`. Grid-cheap builds (~2.2× faster than `Octree3`) + ~1.35-1.41× on clustered cull/k-NN vs the uniform `MortonGrid3` — the pick for skewed data you **rebuild often**; the pointer `Octree3` still wins the queries by ~1.2×. (The k-NN gap was ~5× until `MortonGrid3::knn` got per-axis expansion in 2026-07; re-measured paired, `docs/THREE_D.md`.) |
