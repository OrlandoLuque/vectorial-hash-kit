...
| [SPACE_FILLING_CURVES.md](SPACE_FILLING_CURVES.md) | Morton vs Hilbert, and what a "geohash" actually is (a Morton key with the prefix property). Run counts reproduce Moon et al.'s closed form to within 3 %; Hilbert's advantage **converges** to a constant ~1.9 rather than growing. Why the span scan over-scans 7 299× for either curve, `BIGMIN`/`LITMAX` and the octree decomposition that fix it, and why the key-resolution knob has a floor at 6/π. Plus the radix-trie negative. |
