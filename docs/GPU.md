...
| `gpu_lbvh_query_bench` | **closes the resident loop**: build the LBVH on GPU *and* query it on GPU (stack traversal over the GPU-built `children`/`aabb` buffers) — both **range-count** and **k-NN**, verified == brute force | range-count: 1 M pts, 4 096 q r=24 → GPU **0.32 ms** (12.8 M q/s) vs CPU `Tree3` cull 44.8 ms = **~140×**. **k-NN is the honest inverse**: GPU **~21 ms** vs CPU `Tree3` knn ~9 ms = **~2.3× SLOWER** — a per-thread k-buffer (O(k) find-max/insert) + divergent best-first traversal is GPU-hostile, unlike the branchless range-count reduction. So the GPU broadphase win is **range/count queries**; k-NN stays CPU. |
