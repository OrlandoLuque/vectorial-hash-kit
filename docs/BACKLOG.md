...
4. ~~**★ #161 — use `adaptive_lab` to close #149.**~~ **DONE, and the answer was not the one #149
   ...
   (tighter than any pinned arm, since a pin cannot switch), gives **76–86 % choice, 14–24 % lag**
   across four runs — the step counts (102, 468) are identical every time; only the timed gaps move. Four fifths of the shortfall is the policy asking for the wrong backend, not
...
9. **★ #158 — re-derive `rebuild_query_ratio`. NOW THE MAIN LEAD.** #161 puts **76–86 %** of
...
10. **#149 — close the 0.70×. RE-SCOPED by #161/#167.** The lag it blames is measured at **1–3 %
    of the run**, and only **14–24 %** of the shortfall against a per-act oracle. Do not build the
...
- ~~**#179 The extended-object tax, measured**~~ — **done, and it corrected the advice twice.**
  `examples/extent_tax`: a 10 km world, 50 000 objects (99 % radius 1–5, 0.9 % 20–100, 0.1 %
  500–1 477), queried at radius 500, four arms all answering *whose hull intersects this sphere*
  with the three correct ones asserted to return identical hit counts.
  → **A centre index achieves recall 0.9566** — 234 missed of 5 392. And **94 of the misses are
  SMALL objects** against 118 huge: per object the huge ones are ~1 200× likelier to be missed, but
  a numerous population has numerous boundary cases. So the problem **cannot** be fixed by handling
  the big objects specially, which is what the queued text assumed.
  → **`tiered` (3.38 µs) and `hybrid` (3.35 µs) tie, both 8.5× `enlarged` (28.30 µs).** The win is
  *not enlarging by the global maximum*, not the hybrid as such. Prefer `tiered`: 27.9 candidates per
  query against the hybrid's 527.3 and the global enlargement's 1 282.5.
  → **The scan is forgiving**: the hybrid is still 0.78× the enlarged arm at **16 032** large objects
  and only loses between 16 k and 32 k.
  → Written into `docs/CHOOSING.md` under the point-only precondition. Does NOT need #182: all of
  this is reachable with the point index we already have, which is itself the finding — an
  extent-aware structure would have to beat a tiered point index, not a naive one.
